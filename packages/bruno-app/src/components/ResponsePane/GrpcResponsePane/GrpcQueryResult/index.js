  ...
  if (!hasResponses && !hasError) {
    return (
      <StyledWrapper className="w-full h-full relative flex flex-col">
        <div className="empty-state">{t('RESPONSE_PANE.GRPC.NO_MESSAGES')}</div>
      </StyledWrapper>
    );
  }
    ...
    <StyledWrapper className="w-full h-full relative flex flex-col mt-2" data-testid="grpc-response-content">
                  ...
                  >
                    ...
                    >
                      <span className="message-label">
                        {t('RESPONSE_PANE.GRPC.RESPONSE_MESSAGE_N', { count: originalIndex + 1 })}
                        {index === 0 && <span className="latest-badge">{t('RESPONSE_PANE.GRPC.LATEST')}</span>}
                      </span>
      ...
      {hasError && !hasResponses && !showErrorMessage && (
        <div className="empty-state">
          {t('RESPONSE_PANE.GRPC.NO_MESSAGES_ERROR_DISMISSED')}
        </div>
      )}
    </StyledWrapper>
  );
