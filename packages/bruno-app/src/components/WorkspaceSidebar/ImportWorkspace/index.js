...
const ImportWorkspace = ({ onClose }) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      workspaceLocation: Yup.string().min(1, t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.LOCATION_REQUIRED')).required(t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.LOCATION_REQUIRED'))
    }),
    onSubmit: async (values) => {
      ...
      try {
        ...
        toast.success(t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.SUCCESS'));
        onClose();
      } catch (error) {
        toast.error(multiLineMsg(t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.FAILED'), formatIpcError(error)));
      } finally {
  ...
  const validateAndGetFilePath = (file) => {
    ...
    if (!isZip) {
      toast.error(t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.SELECT_VALID_ZIP'));
      return null;
    }

    const filePath = window?.ipcRenderer?.getFilePath(file);
    if (!filePath) {
      toast.error(t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.FILE_PATH_ERROR'));
      return null;
    }
  ...
  return (
    <Modal
      size="md"
      title={t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.TITLE')}
      confirmText={isSubmitting ? t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.IMPORTING') : t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.IMPORT')}
    ...
    >
      <div className="flex flex-col">
        <div className="mb-4">
          <h3 className="font-semibold mb-2">{t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.WORKSPACE_FILE')}</h3>
          {selectedFile ? (
            <div className="flex items-center justify-between p-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800">
              ...
              >
                {t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.REMOVE')}
              </button>
            </div>
          ) : (
            <div
            ...
            >
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-gray-600 dark:text-gray-300 mb-2">
                  {t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.DROP_FILE')}{' '}
                  <button
                    type="button"
                    className="text-blue-500 underline cursor-pointer"
                    onClick={handleBrowseFiles}
                  >
                    {t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.CHOOSE_FILE')}
                  </button>
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.SUPPORTS_ZIP')}
                </p>
              </div>
            </div>
          )}
        ...
        <div className="mb-4">
          <label htmlFor="workspace-location" className="font-semibold mb-2 flex items-center">
            {t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.EXTRACT_LOCATION')}
            <Help>
              <p>
                {t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.EXTRACT_HINT_1')}
              </p>
              <p className="mt-2">
                {t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.EXTRACT_HINT_2')}
              </p>
            </Help>
          </label>
          <input
          ...
          <div className="mt-1">
            <span
              ...
              onClick={browse}
            >
              {t('WORKSPACE_SIDEBAR.IMPORT_WORKSPACE.BROWSE')}
            </span>
          </div>
        </div>
      </div>
