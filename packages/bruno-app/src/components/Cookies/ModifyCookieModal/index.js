  ...
  const handleCookieDispatch = (cookie, domain, modValues, onClose) => {
    if (cookie) {
      dispatch(modifyCookie(domain, cookie, modValues))
        ...
        .catch((err) => {
          toast.error(t('COOKIES.ERROR_MODIFY_COOKIE'));
          console.error(err);
        });
    } else {
      dispatch(addCookie(domain, modValues))
        ...
        .catch((err) => {
          toast.error(t('COOKIES.ERROR_ADD_COOKIE'));
          console.error(err);
        });
    }
  };
  ...
  const onSubmit = async () => {
    try {
      if (isRawMode) {
        ...
        if (!cookieObj) {
          toast.error(t('COOKIES.VALID_COOKIE_STRING'));
          return;
        }
      ...
      const errMsg = error.message || t('COOKIES.ERROR_PARSE_COOKIE');
      toast.error(errMsg);
    }
  };
          ...
          {isRawMode ? (
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <Tooltip
                  ...
                  html={t('COOKIES.SET_COOKIE_STRING_TOOLTIP')}
                />
              </div>
              <textarea
                value={cookieString}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block mb-1">
                    {t('COOKIES.DOMAIN_LABEL')}<span className="required-asterisk">*</span>{' '}
                  </label>
                  <input
                    type="text"
                    name="domain"
                ...
                <div>
                  <label className="block mb-1">{t('COOKIES.PATH_LABEL')}</label>
                  <input
                    type="text"
                    name="path"
                ...
                <div>
                  <label className="block mb-1">
                    {t('COOKIES.KEY_LABEL')}<span className="required-asterisk">*</span>{' '}
                  </label>
                  <input
                    type="text"
                    name="key"
              ...
              <div className="w-full flex items-end">
                <div>
                  <label className="block mb-1">{t('COOKIES.EXPIRATION_LABEL', { timezone: moment.tz.guess() })}</label>
                  <input
                    type="datetime-local"
                    name="expires"
