  ...
  const persistPreferences = () => {
    if (collectionLocation && collectionLocation !== defaultLocation) {
      ...
      return dispatch(savePreferences(updatedPreferences)).catch(() => {
        toast.error(t('WELCOME_MODAL.FAILED_SAVE_PREFERENCES'));
      });
    }
      ...
      <div className="welcome-card">
        <div className="welcome-header">
          ...
          <h1 className="welcome-heading">
             {step === 1 ? t('WELCOME_MODAL.STEP_1_TITLE') : step === 4 ? t('WELCOME_MODAL.STEP_4_TITLE') : t('WELCOME_MODAL.STEP_2_3_TITLE')}
          </h1>
          {step === 1 && (
          ...
          <div className="footer-buttons">
            ...
            {!isLastStep && (
              <Button type="button" onClick={() => goTo(step + 1)}>
                {step === 1 ? t('WELCOME_MODAL.GET_STARTED') : t('WELCOME_MODAL.NEXT')}
              </Button>
            )}
            {isLastStep && (
              <Button type="button" color="secondary" onClick={handleSaveAndDismiss}>
                {t('WELCOME_MODAL.I_WILL_EXPLORE')}
              </Button>
            )}
          </div>
        </div>
