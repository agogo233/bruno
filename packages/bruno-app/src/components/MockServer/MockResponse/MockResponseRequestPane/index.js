  ...
  return (
    <div className="demo-section">
      ...
      <table className="demo-table w-full">
        <thead>
          <tr>
            <th>{t('MOCK_SERVER.REQUEST_PANE.NAME')}</th>
            <th>{t('MOCK_SERVER.REQUEST_PANE.VALUE')}</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row, index) => (
...
const MockResponseRequestPane = ({
  ...
  const tabConfig = [
    ...
    { name: 'demo-request', label: t('MOCK_SERVER.REQUEST_PANE.DEMO_REQUEST') }
  ];
            ...
            {hasDetails ? (
              <>
                <div className="demo-hint">
                  {t('MOCK_SERVER.REQUEST_PANE.DEMO_HINT')}
                </div>
                {demoRequest.headers.length ? (
                  <DemoKeyValueTable title={t('MOCK_SERVER.REQUEST_PANE.HEADERS')} rows={demoRequest.headers} />
                ) : null}
                {demoRequest.params.length ? (
                  <DemoKeyValueTable title={t('MOCK_SERVER.REQUEST_PANE.QUERY_PARAMS')} rows={demoRequest.params} />
                ) : null}
                {demoRequest.body ? (
                  <div className="demo-section">
                    <div className="demo-section-title">{t('MOCK_SERVER.REQUEST_PANE.BODY')}</div>
              ...
              <div className="demo-hint">
                {t('MOCK_SERVER.REQUEST_PANE.DEMO_NO_RULES', { method: demoRequest.method, url: demoRequest.url })}
              </div>
            )}
          </div>
        );
        ...
        <div className="try-action">
            ...
            items={[
              {
                ...
                label: t('MOCK_SERVER.REQUEST_PANE.OPEN_AS_REQUEST'),
          ...
          >
            <Button
              ...
              aria-label={t('MOCK_SERVER.REQUEST_PANE.MORE_TRY_OPTIONS')}
