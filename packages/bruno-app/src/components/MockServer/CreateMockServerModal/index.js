...
const CreateMockServerModal = ({
  ...
  }), [requiresPortField, existingInstances, editingInstance?.uid, t]);
    ...
    onSubmit: async (values, { setFieldError, setFieldTouched }) => {
      ...
      if (!isEditing && !showAdvancedPort) {
        ...
        try {
          ...
          setFieldError('port', err.message || t('MOCK_SERVER.CREATE_MODAL.PORT_VALIDATE_FAILED'));
          return;
        }
      }
      ...
      if (resolvedSourceType === 'collection' && !selectedCollection) {
        ...
        setFieldError('collectionUid', t('MOCK_SERVER.CREATE_MODAL.COLLECTION_NOT_AVAILABLE'));
        return;
      }
      ...
      try {
        ...
        if (syncError) {
          toast.error(syncError.message || t('MOCK_SERVER.CREATE_MODAL.SYNC_FAILED'));
        } else {
        ...
        >
          ...
          <div className="mt-4">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              <SourceRadio
                value="collection"
                label={t('MOCK_SERVER.CREATE_MODAL.COLLECTION')}
              ...
              <SourceRadio
                value="spec"
                label={t('MOCK_SERVER.CREATE_MODAL.API_SPEC')}
              ...
              <SourceRadio
                value="manual"
                label={t('MOCK_SERVER.CREATE_MODAL.STANDALONE')}
            ...
            <div className="mt-4">
              ...
              {isLinkedCollectionLoading ? (
                <div className="text-xs mt-2 opacity-70" data-testid="mock-server-collection-loading">
                  {t('MOCK_SERVER.CREATE_MODAL.LOADING_COLLECTION')}
                </div>
              ) : null}
            </div>
          ) : null}
          ...
          {formik.values.sourceType === 'manual' ? (
            <div className="mt-4 text-xs opacity-70">
              {t('MOCK_SERVER.CREATE_MODAL.STANDALONE_HINT')}
            </div>
          ) : null}
            ...
            <div className="mt-4">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                ...
                <span className="font-medium">
                  {formik.values.sourceType === 'spec'
                    ? t('MOCK_SERVER.CREATE_MODAL.SYNC_WITH_SPEC')
                    : t('MOCK_SERVER.CREATE_MODAL.SYNC_WITH_EXAMPLES')}
                </span>
              </label>
            </div>
          ) : null}
                  ...
                  <input
                    ...
                    onBlur={async (event) => {
                      ...
                      try {
                        ...
                        formik.setFieldError('port', err.message || t('MOCK_SERVER.CREATE_MODAL.PORT_VALIDATE_FAILED'));
                      }
                    }}
