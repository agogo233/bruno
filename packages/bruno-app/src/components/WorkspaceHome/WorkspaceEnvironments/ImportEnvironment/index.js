...
const ImportEnvironment = ({ onClose, onEnvironmentCreated }) => {
  ...
  const handleImportPostmanEnvironment = () => {
    importPostmanEnvironment()
        ...
        const importPromises = environments
          ...
          .map((environment) =>
            dispatch(addGlobalEnvironment({ name: environment.name, variables: environment.variables }))
              .then(() => {
                toast.success(t('ENVIRONMENTS.IMPORT_MODAL.SUCCESS'));
              })
              .catch((error) => {
      ...
      .catch((err) => toastError(err, t('ENVIRONMENTS.IMPORT_MODAL.POSTMAN_FAILED')));
  };

  return (
    <Portal>
      <Modal size="sm" title={t('ENVIRONMENTS.IMPORT_MODAL.TITLE')} hideFooter={true} handleConfirm={onClose} handleCancel={onClose} dataTestId="import-environment-modal">
        <button
          type="button"
        ...
        >
          ...
          <span className="mt-2 block text-sm font-semibold">{t('ENVIRONMENTS.IMPORT_MODAL.POSTMAN_HINT')}</span>
        </button>
      </Modal>
    </Portal>
  );
