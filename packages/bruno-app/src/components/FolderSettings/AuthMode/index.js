...
const AuthMode = ({ collection, folder }) => {
  ...
  const menuItems = useMemo(() => [
    {
      id: 'awsv4',
      label: t('FOLDER_SETTINGS.AUTH_MODE.AWS_SIG_V4'),
      onClick: () => onModeChange('awsv4')
    },
    {
      id: 'basic',
      label: t('FOLDER_SETTINGS.AUTH_MODE.BASIC_AUTH'),
      onClick: () => onModeChange('basic')
    },
    {
      id: 'bearer',
      label: t('FOLDER_SETTINGS.AUTH_MODE.BEARER_TOKEN'),
      onClick: () => onModeChange('bearer')
    },
    {
      id: 'digest',
      label: t('FOLDER_SETTINGS.AUTH_MODE.DIGEST_AUTH'),
      onClick: () => onModeChange('digest')
    },
    {
      id: 'ntlm',
      label: t('FOLDER_SETTINGS.AUTH_MODE.NTLM_AUTH'),
      onClick: () => onModeChange('ntlm')
    },
    {
      id: 'oauth1',
      label: t('FOLDER_SETTINGS.AUTH_MODE.OAUTH_1'),
      onClick: () => onModeChange('oauth1')
    },
    {
      id: 'oauth2',
      label: t('FOLDER_SETTINGS.AUTH_MODE.OAUTH_2'),
      onClick: () => onModeChange('oauth2')
    },
    {
      id: 'wsse',
      label: t('FOLDER_SETTINGS.AUTH_MODE.WSSE_AUTH'),
      onClick: () => onModeChange('wsse')
    },
    {
      id: 'apikey',
      label: t('FOLDER_SETTINGS.AUTH_MODE.API_KEY'),
    ...
    },
    {
      ...
      label: (
        <span className="flex items-center gap-2">
          {t('FOLDER_SETTINGS.AUTH_MODE.AKAMAI_EDGEGRID')}
          <StatusBadge status="info" size="xs">{t('FOLDER_SETTINGS.AUTH_MODE.BETA')}</StatusBadge>
        </span>
      ),
      ariaLabel: t('FOLDER_SETTINGS.AUTH_MODE.AKAMAI_EDGEGRID') + ' (' + t('FOLDER_SETTINGS.AUTH_MODE.BETA') + ')',
      onClick: () => onModeChange('akamai-edgegrid')
    },
    {
      id: 'inherit',
      label: t('FOLDER_SETTINGS.AUTH_MODE.INHERIT'),
      onClick: () => onModeChange('inherit')
    },
    {
      id: 'none',
      label: t('FOLDER_SETTINGS.AUTH_MODE.NO_AUTH'),
