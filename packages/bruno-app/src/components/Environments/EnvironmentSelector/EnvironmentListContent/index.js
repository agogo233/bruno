    ...
    <div>
      {environments && environments.length > 0 ? (
        <>
          <div className="environment-list">
            <div className="env-list-search">
              <SearchInput
                ref={searchInputRef}
                placeholder={t('ENVIRONMENTS.SELECTOR_LIST.SEARCH_PLACEHOLDER')}
            ...
            >
              ...
              <span>{t('ENVIRONMENTS.SELECTOR_LIST.NO_ENVIRONMENT')}</span>
            </div>
            <ToolHint
              ...
              <div>
                {filteredEnvs.length === 0 && searchText ? (
                  <div className="text-center text-xs opacity-50 py-2 italic" data-testid="env-no-results">
                    {t('ENVIRONMENTS.SELECTOR_LIST.NO_RESULTS')}
                  </div>
                ) : (
            ...
            <div className="dropdown-item configure-button">
              <button onClick={onSettingsClick} id="configure-env" data-testid="configure-env">
                ...
                <span>{t('ENVIRONMENTS.SELECTOR_LIST.CONFIGURE')}</span>
              </button>
            </div>
          </div>
        </>
        ...
        <div className="empty-state">
          <h3>{t('ENVIRONMENTS.SELECTOR_LIST.READY')}</h3>
          <p>{description}</p>
