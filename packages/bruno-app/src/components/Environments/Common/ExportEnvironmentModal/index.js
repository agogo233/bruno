...
const ExportEnvironmentModal = ({ onClose, environments = [], environmentType }) => {
  ...
  const exportFormatOptions = useMemo(() => {
    ...
    if (isMultiple) {
      return [
        { value: 'single-file', label: t('ENVIRONMENTS.EXPORT_MODAL.SINGLE_JSON'), description: t('ENVIRONMENTS.EXPORT_MODAL.SINGLE_JSON_DESC') },
        { value: 'folder', label: t('ENVIRONMENTS.EXPORT_MODAL.SEPARATE_FILES'), description: t('ENVIRONMENTS.EXPORT_MODAL.SEPARATE_FILES_DESC'), disabled: false }
      ];
    }

    return [
      { value: 'single-object', label: t('ENVIRONMENTS.EXPORT_MODAL.SINGLE_JSON'), description: t('ENVIRONMENTS.EXPORT_MODAL.SINGLE_OBJECT_DESC') },
      { value: 'folder', label: t('ENVIRONMENTS.EXPORT_MODAL.SEPARATE_FILES'), description: t('ENVIRONMENTS.EXPORT_MODAL.SEPARATE_FILES_DESC'), disabled: true }
    ];
  ...
  const handleExport = async () => {
    try {
      ...
      if (!filePath) {
        toast.error(t('ENVIRONMENTS.EXPORT_MODAL.SELECT_LOCATION'));
        return;
      }

      if (selectedCount === 0) {
        toast.error(t('ENVIRONMENTS.EXPORT_MODAL.SELECT_ENV'));
        return;
      }
      ...
      const successMessage = exportFormat === 'folder'
        ? t('ENVIRONMENTS.EXPORT_MODAL.SUCCESS_FOLDER', { environmentType })
        : t('ENVIRONMENTS.EXPORT_MODAL.SUCCESS');
      toast.success(successMessage);
      onClose();
    } catch (error) {
      console.error('Export error:', error);
      toast.error(error.message || t('ENVIRONMENTS.EXPORT_MODAL.FAILED'));
    } finally {
  ...
  return (
    <Portal>
      <StyledWrapper>
        <Modal
          size="md"
          title={t('ENVIRONMENTS.EXPORT_MODAL.TITLE')}
          hideFooter={true}
                ...
                <div className="flex flex-col h-full">
                  <div className="flex justify-between items-center mb-2 pb-1">
                    <h3 className="font-medium text-theme">
                      {environmentType === 'global' ? t('ENVIRONMENTS.EXPORT_MODAL.HEADER_GLOBAL') : t('ENVIRONMENTS.EXPORT_MODAL.HEADER_COLLECTION')}
                    </h3>
                    <button
                      type="button"
                    ...
                    >
                      {environments.every((env) => selectedEnvironments[env.uid]) ? t('ENVIRONMENTS.EXPORT_MODAL.DESELECT_ALL') : t('ENVIRONMENTS.EXPORT_MODAL.SELECT_ALL')}
                    </button>
                  </div>
                ...
                <div className="flex flex-col h-full">
                  ...
                  <div className="flex items-center justify-center flex-1 p-4 text-center">
                    <span className="text-xs text-muted">
                      {t('ENVIRONMENTS.EXPORT_MODAL.NO_ENVIRONMENTS', { type: environmentType === 'global' ? 'global' : 'collection' })}
                    </span>
                  </div>
                </div>
              )}
...
{t('ENVIRONMENTS.EXPORT_MODAL.FORMAT')}
                </label>
            ...
            <div className="mb-4">
              <label htmlFor="export-location" className="block font-medium mb-2 text-theme">
                {t('ENVIRONMENTS.EXPORT_MODAL.LOCATION')}
              </label>
              <div className="flex flex-col relative items-center">
                <input
                  ...
                  placeholder={t('ENVIRONMENTS.EXPORT_MODAL.LOCATION_PLACEHOLDER')}
                  autoComplete="off"
                  autoCorrect="off"
                  autoCapitalize="off"
                  spellCheck="false"
            ...
            <div className="flex justify-end gap-2 mt-4 pt-3 border-t border-gray-200 dark:border-gray-700">
              <Button
                ...
                className="mt-2 mr-2"
              >
                {t('ENVIRONMENTS.EXPORT_MODAL.CANCEL')}
              </Button>
              <Button
                type="button"
                size="sm"
                ...
                className="mt-2"
              >
                {isExporting ? t('ENVIRONMENTS.EXPORT_MODAL.EXPORTING') : t(selectedCount === 1 ? 'ENVIRONMENTS.EXPORT_MODAL.EXPORT_SINGLE' : 'ENVIRONMENTS.EXPORT_MODAL.EXPORT_PLURAL', { count: selectedCount })}
              </Button>
            </div>
          </div>
        </Modal>
