    ...
    <StyledWrapper>
      {collectionImportPaths && collectionImportPaths.length > 0 && (
        <div className="content-wrapper">
          <div className="header-wrapper">
            <div className="header-text">{t('REQUEST_PANE.GRPC_QUERY_URL.FROM_COLLECTION_SETTINGS')}</div>
            <button
          ...
          {invalidImportPaths.length > 0 && (
            <div className="error-wrapper">
              <p className="error-text">
                ...
                {t('REQUEST_PANE.GRPC_QUERY_URL.IMPORT_PATHS_NOT_FOUND')}
                {' '}
                <button
                  onClick={onOpenCollectionProtobufSettings}
                  className="error-link"
                >
                  {t('REQUEST_PANE.GRPC_QUERY_URL.MANAGE_IMPORT_PATHS')}
                </button>
              </p>
            </div>
          )}
                    ...
                    <div className="item-left">
                      <div className="checkbox-wrapper">
                        <input
                          ...
                          className="checkbox"
                          title={importPath.enabled ? t('REQUEST_PANE.GRPC_QUERY_URL.IMPORT_PATH_ENABLED') : t('REQUEST_PANE.GRPC_QUERY_URL.IMPORT_PATH_DISABLED')}
                        />
                      </div>
      ...
      {(!collectionImportPaths || collectionImportPaths.length === 0) && (
        <div className="empty-wrapper">
          <div className="empty-text">
            {t('REQUEST_PANE.GRPC_QUERY_URL.NO_IMPORT_PATHS_CONFIGURED')}
          </div>
        </div>
      )}
      ...
      <div className="button-wrapper">
        ...
        >
          ...
          {t('REQUEST_PANE.GRPC_QUERY_URL.BROWSE_IMPORT_PATH')}
        </button>
      </div>
    </StyledWrapper>
  );
