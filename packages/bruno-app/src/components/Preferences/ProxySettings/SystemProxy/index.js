          ...
          <div className="flex flex-row items-center gap-2">
            <div>
              <h2 className="text-xs system-proxy-title flex flex-row">
                {t('PROXY.SYSTEM_PROXY_TITLE')} {isFetching ? <IconLoader2 className="animate-spin ml-1" size={16} strokeWidth={1.5} /> : null}
              </h2>
              <small className="system-proxy-description">
                {t('PROXY.SYSTEM_PROXY_DESC')}
              </small>
            </div>
          </div>
        </div>
        {error && (
          <div className="mb-2 p-3 system-proxy-error-container rounded">
            <small className="system-proxy-error-text">
              {t('PROXY.SYSTEM_PROXY_ERROR')} {error}
            </small>
          </div>
        )}
        {source && (
...
{t('PROXY.REFRESH')}
          ...
          {lastRefreshedAt && (
            ...
            >
              {t('PROXY.LAST_REFRESHED')} {formatProxyTimestamp(lastRefreshedAt)}
            </small>
          )}
        </div>
      </div>
