...
const Beta = ({ close }) => {
  ...
  const handleSave = useCallback((newBetaPreferences) => {
    dispatch(
      ...
      .catch((err) => console.log(err) && toast.error(t('BETA.FAILED_UPDATE')));
  ...
  return (
    <StyledWrapper>
      <div className="section-header">{t('BETA.HEADER')}</div>
            ...
            <div key={feature.id} className="beta-feature-item">
              <div className="beta-feature-header">
                <span className="beta-feature-title select-none font-medium" id={`${feature.id}-label`}>
                  {t(`BETA.FEATURE_${feature.id.replace(/-/g, '_').toUpperCase()}.LABEL`)}
                </span>
              </div>
              <div className="beta-feature-description text-xs text-gray-500 dark:text-gray-400 flex">
                {t(`BETA.FEATURE_${feature.id.replace(/-/g, '_').toUpperCase()}.DESC`)}
                {feature.toggle && (
              ...
              {(feature.action || feature.docsUrl) && (
                <div className="beta-feature-links">
                  {feature.action && (
                    ...
                    >
                      <span>{t(`BETA.FEATURE_${feature.id.replace(/-/g, '_').toUpperCase()}.ACTION`)}</span>
